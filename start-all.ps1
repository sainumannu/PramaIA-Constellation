...
$BACKEND_PORT = [Environment]::GetEnvironmentVariable("BACKEND_PORT")
...
# URL calcolati
$BACKEND_BASE_URL = [Environment]::GetEnvironmentVariable("BACKEND_BASE_URL")
if (-not $BACKEND_BASE_URL) { $BACKEND_BASE_URL = "http://localhost:$BACKEND_PORT" }
...
if (Test-Path "PramaIAServer") {
    ...
    if (Start-PramaService -Name "Backend FastAPI" -Path "." -Command "python -m uvicorn backend.main:app --reload --host 0.0.0.0 --port $BACKEND_PORT" -StartupDelay 8 -Critical) { 
        $criticalServices++
    }
} else {
