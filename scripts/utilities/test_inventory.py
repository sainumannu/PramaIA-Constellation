...
class TestNodeInventory:
    ...
    def test_get_nodes_from_pdk(self):
        ...
        for plugin_id, plugin_nodes in nodes_by_plugin.items():
            ...
            for node in plugin_nodes:
                ...
                print(f"    - {node_type} (in: {inputs}, out: {outputs})")
    ...
    def test_node_schema_validation(self):
        ...
        required_fields = ["nodeType", "inputs", "outputs"]
        invalid_nodes = []
        
        for node in nodes:
            for field in required_fields:
                if field not in node:
                    invalid_nodes.append(f"{node.get('nodeType', 'unknown')}: missing {field}")
        
        if invalid_nodes:
            print("⚠️  Invalid nodes found:")
            for inv in invalid_nodes:
                print(f"  - {inv}")
        else:
            print("✅ All nodes have valid schemas")
