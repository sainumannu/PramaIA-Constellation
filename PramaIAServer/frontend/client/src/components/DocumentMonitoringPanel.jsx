...
import DocumentEventsDetailsPanel from './DocumentEventsDetailsPanel';
...
const DocumentMonitoringPanel = () => {
  ...
  // Stati per la gestione dei dettagli degli eventi dei documenti
  ...
  const fetchPdfEvents = async () => {
    try {
      const response = await fetch(
        `${API_URLS.DATABASE_MANAGEMENT}/document-events/details`, 
        createFetchConfig()
      );
      const data = await response.json();
      setPdfEvents(data);
    } catch (error) {
      console.error('Errore nel recuperare eventi documenti:', error);
  ...
  return (
    <div className="p-6 bg-white rounded shadow max-w-5xl mx-auto space-y-8">
        ...
        <nav className="-mb-px flex">
          ...
          <button
            ...
            onClick={() => {
              ...
              // Carica i dettagli degli eventi documenti se non sono già stati caricati
              if (!pdfEvents) {
                fetchPdfEvents();
              }
            }}
          >
            Statistiche e Analisi Eventi
          </button>
          <button
            className={`py-2 px-4 border-b-2 font-medium text-sm ${
              activeTab === 'sync'
      ...
      {/* Tab Statistiche e Analisi Eventi Documenti */}
      {activeTab === 'pdfEventsDetails' && (
        <DocumentEventsDetailsPanel 
          pdfEvents={pdfEvents}
